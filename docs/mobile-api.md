...
- `POST resources/{resource}`：`action` 支持 `new`、`edit`、`del`、`set`、`addbulk`、`editbulk`、`delbulk`，`data` 为原生 JSON 值。
...
- `GET logs`：`level`、`user`、`search`、`start`、`end`、`offset`、`limit`。结果统一包含系统日志和配置变更审计。
