...
本项目按仓库约定不在开发机生成移动端产物。运行 GitHub Actions 的 `Build S-UI Mobile` 工作流：

- `s-ui-android-arm64`：Android arm64 APK。
- `s-ui-iphone-arm64-unsigned`：未签名 iPhone arm64 IPA，需要使用自己的 Apple Developer 证书签名后安装。
